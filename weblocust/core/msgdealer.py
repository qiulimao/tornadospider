...
class SlaveDetector(object):
    ...
    def __detect(self):
        ...
        nodes = yield RedisConnection.call("smembers",settings.CLUSTER_NODE_SET)
     
        redis_pipeline = RedisPipeline()
        for node in nodes:
            redis_pipeline.stack_call("hget",node,"node_info")
        
        nodes_status = yield RedisConnection().call(redis_pipeline)
        ...
        if invalid_nodes:
            ...
            yield RedisConnection().call(redis_pipeline)
